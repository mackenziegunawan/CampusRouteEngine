...
Algorithm → your Dijkstra implementation + nearest-node/Haversine logic
API → actual endpoints with examples
CLI → actual commands with examples
Project Structure → explain the modules we're looking at
Getting Started → clone/install/run
Example → Klaus → CULC or another recognizable GT route
Technical Decisions → why OSMnx/NetworkX/FastAPI/caching
